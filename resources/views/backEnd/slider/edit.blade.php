...
<div class="page-wrapper">
  <div class="page-content">
      ...
      <div class="ps-3">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb mb-0 p-0">
            ...
            <li class="breadcrumb-item active" aria-current="page"><a href="{{route('admin.slider')}}"></a>All Slider</li>
    ...
    <div class="card">
      <div class="card-body">
        <form method="post" id="update_slider">
        <input type="text" hidden class="form-control " name="id" value="{{$slider->id}}">
          <div class="row">
            <div class="col-md-6 mb-3">
              <label class="form-label required">Slider Title</label>
              <input type="text" class="form-control " name="title" value="{{!empty($slider->title)?$slider->title:''}}">
              <p class="error"></p>
            </div>
            <div class="col-md-6 mb-3">
							<label class="form-label ">Slider link</label>
							<input type="text" class="form-control " name="link" id="link" value="{{!empty($slider->link)?$slider->link:''}}">
							<p class="error"></p>
						</div>
            <div class="col-md-12 mb-3">
              <label for="input23" class="form-label required">Description</label>
              <textarea class="form-control" name="sort_description" id="input23" placeholder="Description..." rows="2">{{!empty($slider->sort_description)?$slider->sort_description:''}}</textarea>
            </div>
            ...
            <div class="col-md-6 mb-3">
              ...
              <div class="input-group">
                <select class="form-select" id="input33" name="status">
                  <option value="active" {{$slider->status == 'active '?'selected':''}}>Show</option>
                  <option value="block" {{$slider->status == 'block'?'selected':''}}>Hide</option>
                </select>
              </div>
            </div>
            ...
            <div class="col-md-6 mb-3">
              ...
              style="max-width: 200px; display: {{$slider->image?'block':'none'}};">
            </div>
            <div class="col-md-12 mb-3">
              <div class="d-md-flex d-grid align-items-center gap-3">
                <button type="submit" class="btn btn-primary px-4"><i class=' bx bxs-check-circle me-0'></i> Update Slider</button>
              </div>
            </div>
