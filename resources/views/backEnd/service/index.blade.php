					...
					<div class="row">
						...
						<div class="col-md-12 mb-3">
							<div class="d-md-flex d-grid align-items-center gap-3">
								<button type="submit" class="btn btn-primary px-4"><i class=' bx bxs-check-circle me-0'></i> Add Service</button>
							</div>
						</div>
					...
					<table id="example2" class="table table-striped table-bordered">
						...
						<tbody>
							...
							<tr id="service_{{$data->id}}">
								<td>{{$key+1}}</td>
								<td><a href="{{route('admin.service.edit',$data->id)}}">{{$data->title}}</a></td>
								...
								<td>
									<div class="d-flex order-actions">
										<a href="{{route('admin.service.edit',$data->id)}}" class=""><i class='bx bx-cog'></i></a>
										<button class="btn delete_service py-0" style="border:none;" value="{{$data->id}}"><i class='bx bx-trash text-danger'></i></button>
									</div>
								</td>
							</tr>
							@endforeach
...
<!-- Add Event form -->
<script>
	$('#add_service').submit(function(event) {
